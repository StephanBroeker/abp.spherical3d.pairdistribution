...
__all__ = ["loadParameterFile", "reconstruct_gUprime", "getUprime","reconstruct_gUprime_vectors"]
    ...
    with open(filepath) as f:
        for line in f:
            ...
            if not (h,j,k) in params:
                params[(h,j,k)] = []
            params[(h,j,k)].append(list(map(float, cells[2:]))) # Extract data
...
def reconstruct_gUprime(
    ...
    if len(np.shape(the1))==3 and len(np.shape(the2))==3 and len(np.shape(phi2))==3:
        pass
    elif len(np.shape(the1)) in [0,1] and len(np.shape(the2)) in [0,1] and len(np.shape(phi2)) in [0,1]:
            ...
            for j in range(lowest_freq,3,1):
                ...
                for params in (
                    ...
                    fitparams = list(map(lambda x : x[0]((Phi, Pe), *x[1]),
                        zip(PARAMETERFITFUNCS[(h,j,k)], params[(h,j,k)])))
                    ...
                    fourierfunc(the1*h) * fourierfunc(the2*j) * phi2_func(phi2*k)
...
def reconstruct_gUprime_vectors(
    ...
    u_1, u_2, u_d: array-like, dimension 3
        Normalized orientation vectors of the particles and their connecting vector
    Phi, Pe: float
        Packing density and Peclet number for which -gU' will be calculated
    params_alpha, params_beta: dict
        Parameter dictionary containing all fit parameters necessary for
